...
//Interrupt management migrated to zephyr
//#include "interrupt_manager.h"
#include <zephyr/irq.h>

#ifndef S32K144_FEATURES_H
// #include "feature.h"
#endif
...
Status_t FLASH_DRV_EnableCmdCompleteInterupt(void)
{
    ...
    //Ported interrupt management to zephyr, Region below modified.
    //INT_SYS_EnableIRQ(flashIrqId);
    irq_enable(flashIrqId);
...
Status_t FLASH_IsFlashAreaReadable(const flash_ssd_config_t * pSSDConfig,
                                              uint32_t dest,
                                              uint32_t size)
{
    uint32_t endAddress = dest+size;
    Status_t ret        = STATUS_SUCCESS;
